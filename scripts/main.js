    ...
    for (let i = 0; i < 9; i++) {
      for (let j = 0; j < 16; j++) {
        for (let k = 0; k < rows[i][j].childNodes.length; k++) {
          if (
            rows[i][j].childNodes[k].classList !== undefined &&
            rows[i][j].childNodes[k].classList.contains("dark")
          ) {
...
function coordsOfTitle(event, color) {
  if (
    event.classList.contains("floor_button") ||
    event.parentNode.classList.contains("floor_button")
  ) {
    let numOfStr = -1;
    for (let i = 1; i < 10; i++) {
      if (
        event.classList.contains(`floor_button_button-line${i}`) ||
        event.parentNode.classList.contains(`floor_button_button-line${i}`)
      ) {
        numOfStr = i;
      }
    }
...
function activeTitileDown(event) {
  ...
  if (
    ...
    target.parentNode.classList.contains("link")
  ) {
...
function colorCells(rownum, j1, color, j2, lasthour, click, meetnum) {
  if (lasthour === true) {
    floorLasthours[rownum].style.background = color;
  }
  if (click === true) {
    meetings[meetnum].style.display = "block";
  }
  for (let j = j1; j <= j2; j++) {
    for (let k = 0; k < rows[rownum][j].childNodes.length; k++) {
      if (
        (rows[rownum][j].childNodes[k].classList !== undefined &&
          rows[rownum][j].childNodes[k].classList.contains(
            "table-hour_3-block"
          )) ||
        (rows[rownum][j].childNodes[k].classList !== undefined &&
          rows[rownum][j].childNodes[k].classList.contains(
...
function bodyClick(event) {
  if (
    event.target.id === "timebar_date1" ||
    event.target.id === "timebar_date2"
  ) {
